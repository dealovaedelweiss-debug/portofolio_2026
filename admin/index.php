<?php
include "config/koneksi.php"; //untuk koneksi 
session_start(); 
session_regenerate_id();
/** @var mysqli $conn */
//untuk menyimpan data sementara

//jika tombol login ditekan dan login adalah name dari button
//ambil nilai email dan password
//post untuk ambil inputan 
if (isset($_POST['login'])) {
  $email = $_POST['email'];
  $pass = sha1($_POST['password']);

  $login = mysqli_query($conn, "SELECT * FROM users WHERE email = '$email'");
  $row = mysqli_fetch_assoc($login); //untuk munculin  dan ambil data
  // var_dump($row); 
  //mysqli_fetch_all($login, MYSQLI_ASSOC); untuk semua data
  

  if ($email == $row['email'] && $pass == $row['password']) {
    $_SESSION['NAME'] = $row['name']; //session disimpan dimemory browser untuk fullstack
    //name pada value adalah yg di db
    // KALAU BERHASIL MASUK KE DASHBOARD
    header("location:dashboard.php");
    // exit();//berhentikan semua proses
  } else { //selain itu
